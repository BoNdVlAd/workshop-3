...
int main(void)
{
  ...
  float avg_volt = 0.0f;
  int count_read = 90;
  volatile HAL_StatusTypeDef adc_pool_result;
  /* USER CODE END 2 */
  /* Infinite loop */
  ...
  while (1)
    	{
  	  	...
  	  	for (int i = 0; i < count_read; i++){
  	  	  	...
  	  	  	adc_pool_result = HAL_ADC_PollForConversion(&hadc1, 100);

  	  	  	if (adc_pool_result == HAL_OK)
  	  	  		 {
  	    ...
  	    adc_avg = adc_sum / (float)count_read;
  	    avg_volt = (adc_avg * 3.3f) / 4096.0f;
